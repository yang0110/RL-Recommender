...
train_data, train_matrix, test_data, test_matrix, user_num, item_num, appropriate_users,test_pic_pool, test_related_pic = preprocess_data(data_dir)
...
class Env():
	def __init__(self, user_item_matrix, user_num, item_num, N, train_matrix=None, test_pic_pool=None, test_related_pic=None):
		self.matrix = user_item_matrix
		self.item_count = item_num
		self.memory = np.ones([user_num, N])*item_num
		self.train_matrix = train_matrix
		self.test_pic_pool = test_pic_pool
		self.test_related_pic = test_related_pic

	def reset(self, user_id):
		self.user_id = user_id 
		self.viewed_items = []
		if self.test_pic_pool is None:
			...
			self.nonrelated_items = np.random.choice(list(set(range(self.item_count))-set(self.related_items)), self.num_rele*2)
			self.available_items = np.zeros(self.num_rele*3)
			self.available_items[::3] = self.related_items
			self.available_items[1::3] = self.nonrelated_items[:self.num_rele]
			self.available_items[2::3] = self.nonrelated_items[self.num_rele:]
		else:
			self.related_items = self.test_related_pic[self.user_id]
			self.available_items = self.test_pic_pool[self.user_id]
...
class Actor_DRR(nn.Module):
	...
	def get_action(self, memory, state_repr, action_emb, items, return_scores=False):
		state = state_repr(memory)
...
def ddpg_update(training_env, step=0, batch_size=params['batch_size'], gamma=params['gamma'], min_value=params['min_value'], max_value=params['max_value'], soft_tau=params['soft_tau']):
	...
	next_state = state_repr(next_memory)
...
def run_evaluation(net, state_repr, training_env_memory, loader):
    ...
    for batch in loader:
        action_emb = net(state_repr(memory))
...
def run_evaluation_2(net, state_repr, training_env_memory):
    ...
    score_list = []
    item_list = []
    test_env = Env(test_matrix, user_num, item_num, params['N'], train_matrix=train_matrix, test_pic_pool=test_pic_pool, test_related_pic=test_related_pic)
    test_env.memory = training_env_memory.copy()
...
value_net = Critic_DRR(params['embedding_dim'], params['embedding_dim'], params['hidden_dim'])
replay_buffer = Prioritized_Buffer(params['buffer_size'])


target_policy_net = Actor_DRR(params['embedding_dim'], params['hidden_dim'])
target_value_net = Critic_DRR(params['embedding_dim'], params['embedding_dim'], params['hidden_dim'])
...
# for u in tqdm.tqdm(users):
	...
	for t in range(int(train_matrix[u].sum())):
		action_emb = policy_net(state_repr(memory))
		if params['ou_noise']:
			action_emb = ou_noise.get_action(action_emb.detach().cpu().numpy()[0], time)
		...
		if step % 5000 == 0 and step > 0:
			...
			auc_list.append(auc)
			ap_list.append(ap)
			print('\n test:  precision, recall, auc, ap', np.round(precison, decimals=3), np.round(recall, decimals=3), np.round(auc, decimals=3), np.round(ap, decimals=3))
...
np.save('../results/ihandy_no_user_precision_list.npy', precison_list)
np.save('../results/ihandy_no_user_ap_list.npy', ap_list)
np.save('../results/ihandy_no_user_auc_list.npy', auc_list)
...
plt.tight_layout()
plt.savefig('../results/run_ihandy_no_user_cum_reward'+'.png', dpi=100)
plt.show()
...
plt.xlabel('training iteration', fontsize=12)
plt.ylabel('test precison', fontsize=12)
plt.tight_layout()
plt.savefig('../results/run_ihandy_no_user_precison'+'.png', dpi=100)
plt.show()
...
plt.tight_layout()
plt.savefig('../results/run_ihandy_no_user_recall'+'.png', dpi=100)
plt.show()
...
plt.tight_layout()
plt.savefig('../results/run_ihandy_no_user_auc'+'.png', dpi=100)
plt.show()
...
plt.tight_layout()
plt.savefig('../results/run_ihandy_no_user_ap'+'.png', dpi=100)
plt.show()


ap_list = np.load('../results/ihandy_no_user_ap_list.npy')
auc_list = np.load('../results/ihandy_no_user_auc_list.npy')

plt.figure(figsize=(6,4))
plt.plot(moving_average(auc_list, n=5), label='AUC')
plt.plot(moving_average(ap_list, n=5), label='MAP')
plt.title('iHandy', fontsize=12)
plt.xlabel('training iteration (x5000)', fontsize=12)
plt.ylabel('Performance', fontsize=12)
plt.legend(loc=2, fontsize=12)
plt.tight_layout()
plt.savefig('../results/run_ihandy_no_user_performance'+'.png', dpi=100)
plt.show()
